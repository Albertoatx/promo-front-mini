...
//import '../styles/misestilos.min.css';
...
angular.module('promotorApp', [
  ...
  .run(['$rootScope', '$log', '$location', 'authFactory', '$interval', '$sce',
    function($rootScope, $log, $location, authFactory, $interval, $sce) { 
     ...
     // $rootScope.googleUrl1 = $sce.trustAsResourceUrl('https://maps.googleapis.com/maps/api/js?key=AIzaSyAYPE5eUENmWQ4ci3EkbTBR3C1c45qb9tA');
      
      //To load on-demand files on partial views (for example for Google Map script)
        ...
        if (url) {
            ...
            if (!script) {
                ...
                if (heads && heads.length) {
                    ...
                    var headString = JSON.stringify(head);
                    //console.log("Heads montado: " + headString);
                }
                //console.log(heads);
            }
            return script;
 ...
 .config(['$routeProvider', 'authFactoryProvider', '$locationProvider',
  function ($routeProvider, authFactoryProvider, $locationProvider) {
    ...
    // For Asynchronous Load of AngularJS Controllers (to improve performance)
    function controllerFactory(ctrl) {
      return {
         // Define property then of our object (which will be used by $q when trying to resolve the promise.
          then: function (done) {
              var self = this;
  
              //Loading our controller with require
              require(['./controllers/' + ctrl], function (ctrl) {
                  self.controller  = ctrl;
                  self.resolve     = ctrl.resolve;
                  self.templateUrl = ctrl.templateUrl;
  
                  done(); //Resolving the promise
              });
          }
      };
    }
    ...
    $routeProvider
      ...
      /*  controller: controllerFactory('promotores'), */
        resolve:{
